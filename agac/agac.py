...
class AGAC(ActorCriticRLModel):
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=linear_schedule(0.001),
                 ...
                 policy_kwargs=None, seed=None, n_cpu_tf_sess=None, rnd_adv=False, add_noise_old=False, add_noise_normal=False):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.compat.v1.variable_scope("loss", reuse=False):
                    ...
                    self.approxkl = .5 * tf.reduce_mean(input_tensor=tf.square(neglogpac - self.old_pi_neglogpac_ph))
                    ...
                    tf.compat.v1.summary.scalar('pi_adv_logits', tf.reduce_mean(input_tensor=train_model.pi_adv_logits))
                    tf.compat.v1.summary.scalar('pi_piold_kl', self.approxkl)
                    tf.compat.v1.summary.scalar('pi_piadv_kl', tf.reduce_mean(input_tensor=l_adv))
                    ...
                    grads1 = tf.gradients(ys=loss, xs=var_list1)
                    if self.max_grad_norm is not None:
                    ...
                    grads2 = tf.gradients(ys=loss, xs=var_list2)
                    if self.max_grad_norm is not None:
                ...
                with tf.compat.v1.variable_scope("input_info", reuse=False):
                    ...
                    tf.compat.v1.summary.scalar('agac_c', tf.reduce_mean(input_tensor=self.agac_c_ph))
                    tf.compat.v1.summary.scalar('advantage', tf.reduce_mean(input_tensor=self.advs_ph))
                    tf.compat.v1.summary.scalar('agac_advantage', tf.reduce_mean(input_tensor=agac_advs_ph))
                    ...
                    tf.compat.v1.summary.scalar('old_neglog_action_probability', tf.reduce_mean(input_tensor=self.old_pi_neglogpac_ph))
