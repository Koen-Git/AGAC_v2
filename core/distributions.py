...
class DiagGaussianProbabilityDistribution(ProbabilityDistribution):
    def __init__(self, flat):
        ...
        self.mean = mean
        # Clip logstd
        self.clipfrac = tf.reduce_mean(input_tensor=tf.cast(tf.greater(LOG_STD_MIN, logstd), tf.float32)) + \
                        tf.reduce_mean(input_tensor=tf.cast(tf.greater(logstd, LOG_STD_MAX), tf.float32))
    ...
    def sample(self):
        ...
        return self.mean + self.std * tf.compat.v1.random_normal(tf.shape(input=self.mean),
